...
public class Runtime_version_6 implements Runnable {
    ...
    public Runtime_version_6(String inputHostName, LinkedBlockingQueue inputRequestQue, ConcurrentHashMap<String, String> inputResultQue, Striped<ReadWriteLock> inputSharedRWLock) {

        mSharedRequestQue = inputRequestQue;
        mSharedResultQue = inputResultQue;
        mReentrantLock = new ReentrantLock();
        mNumberShareResource = new Number_version_6(mReentrantLock);
...
//        System.out.println("size of mRequestQue is " + mSharedRequestQue.size() + ", size of mResultQue is " + mSharedRequestQue.size() + "\n");
        Command_version_6 command;
        try {
            command = (Command_version_6) mSharedRequestQue.take();
    ...
    public String runWorker(String inputWorker, Command_version_6 inputCommand) {
        ...
        if (inputWorker.equals("networkWorker")) {
            ...
            future = executor.submit(
                    () -> {
                        NetworkWorker_version_6 task = new NetworkWorker_version_6(mHOST_NAME, mHOST_SERVER_PORT, inputCommand.getmUThreadID(), "" + inputCommand.getCommand());
                        task.run();
                        return task.call();
                    }
            );
            ...
            future = executor.submit(
                    () -> {
                        LocalWorker_version_6 task = new LocalWorker_version_6(mNumberShareResource, mReentrantLock, inputCommand.getmUThreadID(), inputCommand.getCommand());
                        task.run();
                        return task.call();
                    }
            );
...
//            result = simulate_error(result);
    ...
    public void put_result_into_mResultQue(Command_version_6 inputCommand) {
