...
public class RuntimeThr_version_2 implements Runnable {
    ...
    public RuntimeThr_version_2(String inputHostName) {

        mRequestQue = new LinkedBlockingQueue();
        mResultQue = new LinkedBlockingQueue();
        mLock = new ReentrantLock();
        mNumberShareResource = new Number_version_2(mLock);
    ...
    public void run() {
        ...
        try {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                int id = i + 1;
                new UThr_version_2(id, mRequestQue, mResultQue).run();
            }
            ...
            for (int i = 0; i < 200; i++) {
                Command_version_2 command = (Command_version_2) mRequestQue.peek();
                if (command.getmCommand() == 4 || command.getmCommand() == 5) {
                    try {
                        command = (Command_version_2) mRequestQue.take();
    ...
    public String runLocalThr(int inputClientID, int inputCommand) {
        ...
        LocalThr_version_2 task = new LocalThr_version_2(mNumberShareResource, mLock, inputClientID, inputCommand);
